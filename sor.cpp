...
vector<double> Sor::algorithm() {
    ...
    while(!(this->isFinished(previousIterates, currentIterates))) {
        ...
        currentIterates[0] = previousIterates[0] +
                this->omega * (1./3 * ( -(previousIterates[0] + previousIterates[2] + previousIterates[n-2]) + coefficients[0]));
        currentIterates[1] = previousIterates[1] +
                this->omega * (1./3 * ( -(previousIterates[1] + previousIterates[3] + previousIterates[n-1]) + coefficients[1]));
        for (int i=2; i<n-2; i++){
            currentIterates[i] = previousIterates[i] +
                    this->omega * (1./3 * ( -(currentIterates[i-2] + previousIterates[i] + previousIterates[i+2]) + coefficients[i]));
        }
        currentIterates[n-2] = previousIterates[n-2] +
                this->omega * (1./3 * ( -(currentIterates[0] + currentIterates[n-4] + previousIterates[n-2]) + coefficients[n-2]));
        currentIterates[n-1] = previousIterates[n-1] +
                this->omega * (1./3 * ( -(currentIterates[1] + currentIterates[n-3] + previousIterates[n-1]) + coefficients[n-1]));
    }
